...
class Gifts extends Component {
  ...
  componentDidMount() {
    ...
    API.getList(
      this.props.id
        ? this.props.id
        : this.props.user.wishlists[this.props.user.wishlists.length - 1]
    )
      .then(res => {
  ...
  render() {
    return (
      <Container style={{ paddingTop: "6rem" }} fluid>
        <Row>
          <Col size="m12">
            <div className="gift-jumbo container">
              <Jumbotron>
                <h3>
                  <span role="img" aria-label="gift" className="left emoji">
                    🎁
                  </span>
                  <span> Gift List </span>
                  <span role="img" aria-label="gift" className="right emoji">
                    🎁
                  </span>
                </h3>
              </Jumbotron>
        ...
        <Row>
          {(!this.props.id || this.props.id === this.props.user._id) && (
              <Col size="s12 l4">
                <Collapsible popout defaultActiveKey={1}>
                  <CollapsibleItem header="Add A Gift" icon={<Icon medium>add_box</Icon>}>
                    <Card
                      link={
                        <button
                    ...
                    >
                      <MatRow style={{ flex: "none", display: "block" }}>
                        ...
                        <TextInput
                          ...
                          name="descr"
                        />
                        <button className="btn btn-large" onClick={this._scan}>
                          {this.state.scanning
                            ? "Click When Done"
                            : "Scan a Barcode"}
                        </button>
          ...
          <Col
            size="s12 l8"
            style={{ margin: "0 auto", flexGrow: 0, flexBasis: "auto" }}
          >
            <Camera
              scanning={this.state.scanning}
              scanner={this.scanner.bind(this)}
            />
              ...
              >
                ...
                {(!this.props.id || this.props.id === this.props.user._id) && (
                  <span
                    ...
                    className="right"
                  >
                    {item.status === "Open" ? "Not Purchased!" : "Purchased"}
                  </span>
                )}
                <br />
                <br />
                <span style={{ position: "absolute", bottom: 15, left: 14 }}>
                  ${item.price}
                </span>
                <div
                  ...
                  {!this.props.id || this.props.id === this.props.user._id ? (
                    <div>
                      {item.status === "Open" ? (
                        <button
                          style={{ fontSize: 18 }}
                          onClick={() => this.deleteGift(item._id)}
                        >
                          Delete Item
                        </button>
                      ) : null}
                    ...
                    <Link
                      to={
                        item.status === "Open" ? "/giftdetail/" + item._id : "#"
                      }
                    >
                      <button
                        className={
